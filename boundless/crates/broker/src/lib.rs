...
pub(crate) mod onchain_market_monitor;
    ...
    P: Provider<Ethereum> + 'static + Clone + WalletProvider,
{
    ...
    pub async fn start_service(&self) -> Result<()> {
        ...
        // spin up a supervisor for the on-chain market monitor (WS events)
        {
            // Build a dedicated WS provider for event subscription using config.network.ws_rpc_url
            use alloy::providers::ProviderBuilder;

            let ws_url = {
                let cfg = config.lock_all().unwrap();
                cfg.network.ws_rpc_url.clone()
            };
            ...
            let onchain_monitor = Arc::new(onchain_market_monitor::OnchainMarketMonitor::new(
                ws_provider,
                self.deployment().boundless_market_address,
                chain_id,
                new_order_tx.clone(),
            ...
            let cancel_token = non_critical_cancel_token.clone();
            supervisor_tasks.spawn(async move {
                Supervisor::new(onchain_monitor, cloned_config, cancel_token)
                    .spawn()
                    .await
                    .context("Failed to start on-chain market monitor")?;
                Ok(())
            });
        }
